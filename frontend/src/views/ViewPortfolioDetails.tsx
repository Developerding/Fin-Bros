...
const ViewPortfolioDetails = () => {
    ...
    return AppStore.viewStockController(stockTicker)
      .then((res) => {
        ...
        setStocks((prevStocks) => {
          ...
          const isStockExists = prevStocks.some(
            (stock) => stock.name === res.name
          );
  ...
  const allocationData = stockPortfolioData.allocations?.map((allocation) => {
    ...
    const numberOfSharesBought = Math.round(
      capitalAllocated / allocation.averagePrice
    );
    const returns = Math.round(
      (currentStockPrice - allocation.averagePrice) *
        (capitalAllocated / allocation.averagePrice)
    );
  ...
  return (
    <>
      ...
      <Container maxWidth="xl" sx={{ marginTop: "2%" }}>
              ...
              <CardContent>
                ...
                >
                  ...
                  <Typography sx={{ paddingY: 1 }}>
                    Portfolio consists of stocks from{" "}
                    {stockPortfolioData.allocations?.map(
                      (allocation: any, index: number) => (
                        <span key={index}>
                          ...
                          {index === stockPortfolioData.allocations.length - 1
                            ? ""
                            : index ===
                              stockPortfolioData.allocations.length - 2
                            ? " and "
                            : ", "}
                        </span>
                      )
                  ...
                  <Typography>
                    Total capital allocated to this portfolio is{" "}
                    <b>${stockPortfolioData.capital}</b>.
                  </Typography>
                </Stack>
              </CardContent>
            </Card>
                  ...
                  >
                    <Grid item>
                      ...
                      <AllocationChart
                        allocations={
                          stockPortfolioData
                            ? stockPortfolioData.allocations
                            : null
                        }
                      ></AllocationChart>
                    </Grid>
                            ...
                            (allocation: any, index: number) => (
                              ...
                              >
                                <TableCell align="center">
                                  <Avatar
                                    src={`/assets/stocks/${allocation.stockName}.png`}
                          ...
                          {allocationData?.map(
                            (allocation: any, index: number) => (
                              <TableRow key={index}>
                                ...
                                <TableCell align="center">
                                  {typeof allocation.currentStockPrice ==
                                  "number"
                                    ? allocation.currentStockPrice
                                    : "loading"}
                                </TableCell>
                                <TableCell align="center">
                                  {typeof allocation.currentStockPrice ==
                                ...
                                <TableCell
                                  ...
                                  sx={{
                                    fontWeight: "bold",
                                    color: allocation.isPositiveReturn
                                      ? "green"
                                      : "red",
                                  }}
                                >
        ...
        <Grid container spacing={4} sx={{ marginTop: "1%" }}></Grid>
      </Container>
    </>
  );
};
